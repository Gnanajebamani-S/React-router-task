function CyberSecurity() {
  return (
    <>
      <div className="container">
        <h1 className="text-center" style={{ marginTop: 60 }}>
          CYBER SECURITY
        </h1>
        <div className="row">
          <div className="col-3">
            <div className="post-card trending-card">
              <div className="card border-0 shadow-sm position-relative">
                <a
                  href="https://www.guvi.in/blog/how-to-render-an-array-of-objects-in-react/"
                  className="card-img-top"
                >
                  <img
                    width="300"
                    height="200"
                    src="https://www.guvi.in/blog/wp-content/uploads/2022/12/Render-array-of-objects-in-ReactJs.png"
                    className="attachment-post-thumbnail size-post-thumbnail wp-post-image"
                    alt="Array of Objects"
                    decoding="async"
                  />{" "}
                </a>
                <div className="card-body" id="card-details">
                  <a
                    className="card-title text-decoration-none text-dark fs-5 fw-semibold"
                    href="https://www.guvi.in/blog/how-to-render-an-array-of-objects-in-react/"
                  >
                    How to Render an Array of Objects in React? [in 3 easy
                    steps]{" "}
                  </a>
                  <p className="card-author mt-2 mb-0">
                    <span id="by">By</span>&nbsp;
                    <a
                      className="text-decoration-none"
                      href="https://www.guvi.in/blog/author/tarun/"
                    >
                      Tarun Singh
                    </a>
                  </p>
                  <div
                    id="card-footer"
                    className="mt-2 d-flex flex-row flex-nowrap justify-content-between"
                  >
                    <p className="mb-0 card-text">21 Mar, 2024</p>
                    <p className="mb-0 card-text">7 Min Read </p>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className="col-3">
            <div className="post-card trending-card">
              <div className="card border-0 shadow-sm position-relative">
                <a
                  href="https://www.guvi.in/blog/how-to-fetch-data-using-api-in-react/"
                  className="card-img-top"
                >
                  <img
                    width="300"
                    height="200"
                    src="https://www.guvi.in/blog/wp-content/uploads/2022/12/Fetch-Data-using-API-in-React.Js.png"
                    className="attachment-post-thumbnail size-post-thumbnail wp-post-image"
                    alt="React"
                    decoding="async"
                  />{" "}
                </a>
                <div className="card-body" id="card-details">
                  <a
                    className="card-title text-decoration-none text-dark fs-5 fw-semibold"
                    href="https://www.guvi.in/blog/how-to-fetch-data-using-api-in-react/"
                  >
                    Use ReactJS to Fetch and Display Data from API – 5 Simple
                    Steps{" "}
                  </a>
                  <p className="card-author mt-2 mb-0">
                    <span id="by">By</span>&nbsp;
                    <a
                      className="text-decoration-none"
                      href="https://www.guvi.in/blog/author/tarun/"
                    >
                      Tarun Singh
                    </a>
                  </p>
                  <div
                    id="card-footer"
                    className="mt-2 d-flex flex-row flex-nowrap justify-content-between"
                  >
                    <p className="mb-0 card-text">21 Mar, 2024</p>
                    <p className="mb-0 card-text">5 Min Read </p>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className="col-3">
            <div className="post-card trending-card">
              <div className="card border-0 shadow-sm position-relative">
                <a
                  href="https://www.guvi.in/blog/unique-project-ideas-for-college-students/"
                  className="card-img-top"
                >
                  <img
                    width="300"
                    height="200"
                    src="https://www.guvi.in/blog/wp-content/uploads/2023/06/feature-image-unique-project-ideas.jpg"
                    className="attachment-post-thumbnail size-post-thumbnail wp-post-image"
                    alt="feature image-unique project ideas"
                    decoding="async"
                  />{" "}
                </a>
                <div className="card-body" id="card-details">
                  <a
                    className="card-title text-decoration-none text-dark fs-5 fw-semibold"
                    href="https://www.guvi.in/blog/unique-project-ideas-for-college-students/"
                  >
                    Top 10 Unique Project Ideas for College Students{" "}
                  </a>
                  <p className="card-author mt-2 mb-0">
                    <span id="by">By</span>&nbsp;
                    <a
                      className="text-decoration-none"
                      href="https://www.guvi.in/blog/author/lukesh/"
                    >
                      Lukesh S
                    </a>
                  </p>
                  <div
                    id="card-footer"
                    className="mt-2 d-flex flex-row flex-nowrap justify-content-between"
                  >
                    <p className="mb-0 card-text">21 Mar, 2024</p>
                    <p className="mb-0 card-text">7 Min Read </p>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className="col-3">
            <div className="post-card trending-card">
              <div className="card border-0 shadow-sm position-relative">
                <a
                  href="https://www.guvi.in/blog/mechanical-engineering-project-ideas/"
                  className="card-img-top"
                >
                  <img
                    width="300"
                    height="200"
                    src="https://www.guvi.in/blog/wp-content/uploads/2023/09/Feature-image-Mechanical-Engineering-Project-Ideas.webp"
                    className="attachment-post-thumbnail size-post-thumbnail wp-post-image"
                    alt="Feature image -Mechanical Engineering Project Ideas"
                    decoding="async"
                  />{" "}
                </a>
                <div className="card-body" id="card-details">
                  <a
                    className="card-title text-decoration-none text-dark fs-5 fw-semibold"
                    href="https://www.guvi.in/blog/mechanical-engineering-project-ideas/"
                  >
                    20 Mechanical Engineering Project Ideas{" "}
                  </a>
                  <p className="card-author mt-2 mb-0">
                    <span id="by">By</span>&nbsp;
                    <a
                      className="text-decoration-none"
                      href="https://www.guvi.in/blog/author/meghana/"
                    >
                      Meghana D
                    </a>
                  </p>
                  <div
                    id="card-footer"
                    className="mt-2 d-flex flex-row flex-nowrap justify-content-between"
                  >
                    <p className="mb-0 card-text">25 Mar, 2024</p>
                    <p className="mb-0 card-text">9 Min Read </p>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </>
  );
}

export default CyberSecurity;
