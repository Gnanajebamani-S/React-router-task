import FSDprojectideas from "./images/FSDprojectideas.jpg";
import apache from "./images/apache.png";
import fsdSyllabus from "./images/fsdSyllabus.png";
import Reactjs from "./images/Reactjs.png";
import database from "./images/database.png";
import Roadmap from "./images/Roadmap.png";
import framework from "./images/framework.png";
function Fsd() {
  return (
    <>
      <div className="container">
        <h1 className="text-center" style={{ marginTop: 60 }}>
          FULL STACK DEVELOPMENT
        </h1>
        <div className="row">
          <div className="col-3">
            <div className="post-card trending-card">
              <div className="card border-0 shadow-sm position-relative">
                <a
                  href="https://www.guvi.in/blog/full-stack-development-project-ideas/"
                  className="card-img-top"
                >
                  <img
                    width="300"
                    height="200"
                    src={FSDprojectideas}
                    className="attachment-post-thumbnail size-post-thumbnail wp-post-image"
                    alt="Best Full Stack Development Project Ideas"
                    decoding="async"
                  />{" "}
                </a>

                <div className="card-body" id="card-details">
                  <a
                    className="card-title text-decoration-none text-dark fs-5 fw-semibold"
                    href="https://www.guvi.in/blog/full-stack-development-project-ideas/"
                  >
                    Best Full-Stack Development Project Ideas in 2024{" "}
                  </a>
                  <p className="card-author mt-2 mb-0">
                    <span id="by">By</span>&nbsp;
                    <a
                      className="text-decoration-none"
                      href="https://www.guvi.in/blog/author/isha/"
                    >
                      Isha Sharma
                    </a>
                  </p>
                  <div
                    id="card-footer"
                    className="mt-2 d-flex flex-row flex-nowrap justify-content-between"
                  >
                    <p className="mb-0 card-text">25 Mar, 2024</p>
                    <p className="mb-0 card-text">4 Min Read </p>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className="col-3">
            <div className="post-card trending-card">
              <div className="card border-0 shadow-sm position-relative">
                <a
                  href="https://www.guvi.in/blog/how-long-would-it-take-to-be-a-full-stack-developer/"
                  className="card-img-top"
                >
                  <img
                    width="300"
                    height="200"
                    src="https://www.guvi.in/blog/wp-content/uploads/2023/07/how-long-it-would-it-take-to-learn-full-stack-development_.webp"
                    className="attachment-post-thumbnail size-post-thumbnail wp-post-image"
                    alt=""
                    decoding="async"
                  />{" "}
                </a>
                <div className="card-body" id="card-details">
                  <a
                    className="card-title text-decoration-none text-dark fs-5 fw-semibold"
                    href="https://www.guvi.in/blog/how-long-would-it-take-to-be-a-full-stack-developer/"
                  >
                    How Long Would It Take to Be a Full Stack Developer?{" "}
                  </a>
                  <p className="card-author mt-2 mb-0">
                    <span id="by">By</span>&nbsp;
                    <a
                      className="text-decoration-none"
                      href="https://www.guvi.in/blog/author/meghana/"
                    >
                      Meghana D
                    </a>
                  </p>
                  <div
                    id="card-footer"
                    className="mt-2 d-flex flex-row flex-nowrap justify-content-between"
                  >
                    <p className="mb-0 card-text">26 Mar, 2024</p>
                    <p className="mb-0 card-text">3 Min Read </p>
                  </div>
                </div>
                <div className="position-absolute" id="views-section">
                  <div className="card-views d-flex px-2 py-1 align-items-center bg-white rounded-pill"></div>
                </div>
              </div>
            </div>
          </div>
          <div className="col-3">
            <div className="post-card trending-card">
              <div className="card border-0 shadow-sm position-relative">
                <a
                  href="https://www.guvi.in/blog/how-does-apache-work/"
                  className="card-img-top"
                >
                  <img
                    width="300"
                    height="200"
                    src={apache}
                    className="attachment-post-thumbnail size-post-thumbnail wp-post-image"
                    alt=""
                    decoding="async"
                  />{" "}
                </a>
                <div className="card-body" id="card-details">
                  <a
                    className="card-title text-decoration-none text-dark fs-5 fw-semibold"
                    href="https://www.guvi.in/blog/how-does-apache-work/"
                  >
                    How does Apache work? A detailed introduction to Apache{" "}
                  </a>
                  <p className="card-author mt-2 mb-0">
                    <span id="by">By</span>&nbsp;
                    <a
                      className="text-decoration-none"
                      href="https://www.guvi.in/blog/author/tushar/"
                    >
                      Tushar Vinocha
                    </a>
                  </p>
                  <div
                    id="card-footer"
                    className="mt-2 d-flex flex-row flex-nowrap justify-content-between"
                  >
                    <p className="mb-0 card-text">26 Mar, 2024</p>
                    <p className="mb-0 card-text">5 Min Read </p>
                  </div>
                </div>
                <div className="position-absolute" id="views-section">
                  <div className="card-views d-flex px-2 py-1 align-items-center bg-white rounded-pill"></div>
                </div>
              </div>
            </div>
          </div>
          <div className="col-3">
            <div className="post-card trending-card">
              <div className="card border-0 shadow-sm position-relative">
                <a
                  href="https://www.guvi.in/blog/full-stack-developer-syllabus/"
                  className="card-img-top"
                >
                  <img
                    width="300"
                    height="200"
                    src={fsdSyllabus}
                    className="attachment-post-thumbnail size-post-thumbnail wp-post-image"
                    alt="Full Stack Developer course syllabus 2022"
                    decoding="async"
                  />{" "}
                </a>
                <div className="card-body" id="card-details">
                  <a
                    className="card-title text-decoration-none text-dark fs-5 fw-semibold"
                    href="https://www.guvi.in/blog/full-stack-developer-syllabus/"
                  >
                    Hot Topics That You Need To Know In Full Stack Developer
                    Syllabus{" "}
                  </a>
                  <p className="card-author mt-2 mb-0">
                    <span id="by">By</span>&nbsp;
                    <a
                      className="text-decoration-none"
                      href="https://www.guvi.in/blog/author/archana/"
                    >
                      Archana
                    </a>
                  </p>
                  <div
                    id="card-footer"
                    className="mt-2 d-flex flex-row flex-nowrap justify-content-between"
                  >
                    <p className="mb-0 card-text">26 Mar, 2024</p>
                    <p className="mb-0 card-text">6 Min Read </p>
                  </div>
                </div>
                <div className="position-absolute" id="views-section">
                  <div className="card-views d-flex px-2 py-1 align-items-center bg-white rounded-pill"></div>
                </div>
              </div>
            </div>
          </div>
          <div className="col-3">
            <div className="post-card trending-card">
              <div className="card border-0 shadow-sm position-relative">
                <a
                  href="https://www.guvi.in/blog/prerequisites-for-reactjs/"
                  className="card-img-top"
                >
                  <img
                    width="300"
                    height="200"
                    src={Reactjs}
                    className="attachment-post-thumbnail size-post-thumbnail wp-post-image"
                    alt="react"
                  />{" "}
                </a>
                <div className="card-body" id="card-details">
                  <a
                    className="card-title text-decoration-none text-dark fs-5 fw-semibold"
                    href="https://www.guvi.in/blog/prerequisites-for-reactjs/"
                  >
                    6 Essential Prerequisites For Learning ReactJS{" "}
                  </a>
                  <p className="card-author mt-2 mb-0">
                    <span id="by">By</span>&nbsp;
                    <a
                      className="text-decoration-none"
                      href="https://www.guvi.in/blog/author/ramkumar/"
                    >
                      Ramkumar
                    </a>
                  </p>
                  <div
                    id="card-footer"
                    className="mt-2 d-flex flex-row flex-nowrap justify-content-between"
                  >
                    <p className="mb-0 card-text">25 Mar, 2024</p>
                    <p className="mb-0 card-text">3 Min Read </p>
                  </div>
                </div>
                <div className="position-absolute" id="views-section">
                  <div className="card-views d-flex px-2 py-1 align-items-center bg-white rounded-pill"></div>
                </div>
              </div>
            </div>
          </div>
          <div className="col-3">
            <div className="post-card trending-card">
              <div className="card border-0 shadow-sm position-relative">
                <a
                  href="https://www.guvi.in/blog/database-management-systems-for-developers/"
                  className="card-img-top"
                >
                  <img
                    width="300"
                    height="200"
                    src={database}
                    className="attachment-post-thumbnail size-post-thumbnail wp-post-image"
                    alt="database management system"
                  />{" "}
                </a>
                <div className="card-body" id="card-details">
                  <a
                    className="card-title text-decoration-none text-dark fs-5 fw-semibold"
                    href="https://www.guvi.in/blog/database-management-systems-for-developers/"
                  >
                    10 Best Database Management Systems For Software Developers{" "}
                  </a>
                  <p className="card-author mt-2 mb-0">
                    <span id="by">By</span>&nbsp;
                    <a
                      className="text-decoration-none"
                      href="https://www.guvi.in/blog/author/isha/"
                    >
                      Isha Sharma
                    </a>
                  </p>
                  <div
                    id="card-footer"
                    className="mt-2 d-flex flex-row flex-nowrap justify-content-between"
                  >
                    <p className="mb-0 card-text">22 Mar, 2024</p>
                    <p className="mb-0 card-text">4 Min Read </p>
                  </div>
                </div>
                <div className="position-absolute" id="views-section">
                  <div className="card-views d-flex px-2 py-1 align-items-center bg-white rounded-pill"></div>
                </div>
              </div>
            </div>
          </div>
          <div className="col-3">
            <div className="post-card trending-card">
              <div className="card border-0 shadow-sm position-relative">
                <a
                  href="https://www.guvi.in/blog/web-development-roadmap-for-beginners/"
                  className="card-img-top"
                >
                  <img
                    width="300"
                    height="200"
                    src={Roadmap}
                    className="attachment-post-thumbnail size-post-thumbnail wp-post-image"
                    alt="web development roadmap"
                  />{" "}
                </a>
                <div className="card-body" id="card-details">
                  <a
                    className="card-title text-decoration-none text-dark fs-5 fw-semibold"
                    href="https://www.guvi.in/blog/web-development-roadmap-for-beginners/"
                  >
                    Best Web Development Roadmap for Beginners 2024{" "}
                  </a>
                  <p className="card-author mt-2 mb-0">
                    <span id="by">By</span>&nbsp;
                    <a
                      className="text-decoration-none"
                      href="https://www.guvi.in/blog/author/isha/"
                    >
                      Isha Sharma
                    </a>
                  </p>
                  <div
                    id="card-footer"
                    className="mt-2 d-flex flex-row flex-nowrap justify-content-between"
                  >
                    <p className="mb-0 card-text">14 Dec, 2023</p>
                    <p className="mb-0 card-text">5 Min Read </p>
                  </div>
                </div>
                <div className="position-absolute" id="views-section">
                  <div className="card-views d-flex px-2 py-1 align-items-center bg-white rounded-pill"></div>
                </div>
              </div>
            </div>
          </div>
          <div className="col-3">
            <div className="post-card trending-card">
              <div className="card border-0 shadow-sm position-relative">
                <a
                  href="https://www.guvi.in/blog/best-full-stack-development-online-courses/"
                  className="card-img-top"
                >
                  <img
                    width="300"
                    height="200"
                    src="https://www.guvi.in/blog/wp-content/uploads/2023/08/Feature-image-Best-Full-Stack-Development-Online-Courses.webp"
                    className="attachment-post-thumbnail size-post-thumbnail wp-post-image"
                    alt="Feature image - Best Full-Stack Development Online Courses"
                  />{" "}
                </a>
                <div className="card-body" id="card-details">
                  <a
                    className="card-title text-decoration-none text-dark fs-5 fw-semibold"
                    href="https://www.guvi.in/blog/best-full-stack-development-online-courses/"
                  >
                    7 Best Full-Stack Development Online Courses [2024]{" "}
                  </a>
                  <p className="card-author mt-2 mb-0">
                    <span id="by">By</span>&nbsp;
                    <a
                      className="text-decoration-none"
                      href="https://www.guvi.in/blog/author/srinithi/"
                    >
                      Srinithi Sankar
                    </a>
                  </p>
                  <div
                    id="card-footer"
                    className="mt-2 d-flex flex-row flex-nowrap justify-content-between"
                  >
                    <p className="mb-0 card-text">25 Mar, 2024</p>
                    <p className="mb-0 card-text">5 Min Read </p>
                  </div>
                </div>
                <div className="position-absolute" id="views-section">
                  <div className="card-views d-flex px-2 py-1 align-items-center bg-white rounded-pill"></div>
                </div>
              </div>
            </div>
          </div>
          <div className="col-3">
            <div className="post-card trending-card">
              <div className="card border-0 shadow-sm position-relative">
                <a
                  href="https://www.guvi.in/blog/full-stack-developer-frameworks/"
                  className="card-img-top"
                >
                  <img
                    width="300"
                    height="200"
                    src={framework}
                    className="attachment-post-thumbnail size-post-thumbnail wp-post-image"
                    alt="Top Full Stack Developer Frameworks"
                  />{" "}
                </a>
                <div className="card-body" id="card-details">
                  <a
                    className="card-title text-decoration-none text-dark fs-5 fw-semibold"
                    href="https://www.guvi.in/blog/full-stack-developer-frameworks/"
                  >
                    Top 10 Full-Stack Developer Frameworks in 2024{" "}
                  </a>
                  <p className="card-author mt-2 mb-0">
                    <span id="by">By</span>&nbsp;
                    <a
                      className="text-decoration-none"
                      href="https://www.guvi.in/blog/author/isha/"
                    >
                      Isha Sharma
                    </a>
                  </p>
                  <div
                    id="card-footer"
                    className="mt-2 d-flex flex-row flex-nowrap justify-content-between"
                  >
                    <p className="mb-0 card-text">13 Dec, 2023</p>
                    <p className="mb-0 card-text">4 Min Read </p>
                  </div>
                </div>
                <div className="position-absolute" id="views-section">
                  <div className="card-views d-flex px-2 py-1 align-items-center bg-white rounded-pill"></div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </>
  );
}

export default Fsd;
